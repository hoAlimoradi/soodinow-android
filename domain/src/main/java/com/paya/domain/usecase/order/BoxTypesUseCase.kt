...
class BoxTypesUseCase @Inject constructor(
	private val orderRepository: LowRiskInvestmentRepository
): UseCase<Unit, BoxTypeRepoModel> {
	override suspend fun action(param: Unit): Resource<BoxTypeRepoModel> {
		return orderRepository.boxTypes()
	}
}